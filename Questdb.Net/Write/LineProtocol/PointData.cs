...
namespace Questdb.Net.Write
{
    public class PointData : IEquatable<PointData>
    {
        ...
        {
            if (timestamp != DateTime.MinValue && timestamp.Kind != DateTimeKind.Utc)
            {
        ...
        /// <param name="timeUnit">the timestamp precision</param>
        /// <returns></returns>
        public PointData Timestamp(OffsetDateTime timestamp, WritePrecision timeUnit)
        {
            BigInteger? time = null;
            switch (timeUnit)
            {
                case WritePrecision.Seconds:
                    time = timestamp.ToInstant().ToUnixTimeSeconds();
                    break;
                case WritePrecision.Milliseconds:
                    time = timestamp.ToInstant().ToUnixTimeMilliseconds();
                    break;
                case WritePrecision.Microseconds:
                    time = (long)(timestamp.ToInstant().ToUnixTimeTicks() * 0.1);
                    break;
                case WritePrecision.Nanoseconds:
                    time = (long)(timestamp.ToInstant().ToUnixTimeTicks() * 0.1);
                    break;
